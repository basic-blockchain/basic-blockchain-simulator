...
| BR-WL-10 | Treasury → user transfers carry the sentinel `signature == "TREASURY"` (`TREASURY_SIGNATURE` in `domain/wallet.py`). The chain validator skips ECDSA verification for this sentinel exactly as it does for `COINBASE_SIGNATURE`, because the simulator does **not** persist a treasury wallet's private key (`WalletService.generate_wallet_material` discards the seed once the public key is derived). Authorisation is enforced upstream by the Phase 7.8 dual-sign envelope (BR-TR-01..BR-TR-09): a transfer reaches the mempool only after two distinct admins approve, and the approving admin's identity is captured in `treasury_distributions.approved_by` + `audit_log`. Balance deltas follow the regular transfer branch in `apply_block_deltas`. | `domain/wallet.py` (`TREASURY_SIGNATURE`, `apply_block_deltas`), `domain/blockchain.py` (`_validate_blocks`), `services/treasury_distribution_service.py` (Phase 7.8.3) |
...
| BR-TR-11 | `APPROVE_TREASURY_MINT_OP` is **not** in any role baseline (parity with BR-WL-07 / `MINT`). Approving a dual-sign mint modifies token supply identically to a synchronous mint, so it requires an explicit per-admin grant via `MANAGE_PERMISSIONS` → `user_permissions`, which writes an `audit_log` row for every elevation. `APPROVE_TREASURY_DISTRIBUTION` stays in the ADMIN baseline — distribution moves existing tokens between wallets without touching supply. | `domain/permissions.py` (`ROLE_PERMISSIONS`), `migrations/V023` |
